...
//' @keywords internal
// [[Rcpp::export]]
IntegerMatrix fwHMM_wrapper(SEXP X_, SEXP pInit_, SEXP Q_, SEXP pEmit_,
                            SEXP n_, SEXP p_, SEXP K_, SEXP M_,
                            SEXP seed_, SEXP G_, SEXP display_progress_) {
  int n = as<int>(n_);
  int p = as<int>(p_);
  ...
  for(int i=0; i<n; i++) {
    if (Progress::check_abort() )
      return (Xk_);
    Xk = knock.sample_fb(ivector(X(i,_).begin(),X(i,_).end()));
    for(int j=0; j<p; j++) {
      Xk_(i,j) = Xk[j];
    }
    progr.increment();
...
//' @keywords internal
// [[Rcpp::export]]
IntegerMatrix fwHaplotypes_wrapper(SEXP X_, SEXP r_, SEXP alpha_, SEXP theta_, SEXP groups_,
                                   SEXP n_, SEXP p_, SEXP seed_, SEXP display_progress_) {
  int n = as<int>(n_);
  int p = as<int>(p_);
  IntegerMatrix X = as<IntegerMatrix>(X_);
...
//' @keywords internal
// [[Rcpp::export]]
IntegerMatrix fwGenotypes_wrapper(SEXP X_, SEXP r_, SEXP alpha_, SEXP theta_, SEXP groups_,
                                   SEXP n_, SEXP p_, SEXP seed_, SEXP display_progress_) {
  int n = as<int>(n_);
  int p = as<int>(p_);
  IntegerMatrix X = as<IntegerMatrix>(X_);
